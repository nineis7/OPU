...
void OPUShortInsn::Decode(OPURegFile* reg) {
  ...
  switch (opcode) {
    ...
    case 10:
      ...
      break;
    case 11:
      reg->ddr_load_single = static_cast<bool>(BitSelect(*data_, 30));
      reg->ker_on_board = static_cast<int>(BitSelect(*data_, 29, 24));
      reg->ddr_load_start_trig = static_cast<int>(BitSelect(*data_, 23, 21));
      reg->ddr_load_start_dma_num = static_cast<int>(BitSelect(*data_, 20, 12));
      reg->ddr_load_type.clear();
      if (BitSelect(*data_, 11, 6) & 0x1) {
        reg->ddr_load_type.push_back(OPU_MEM_ID_FM);
      }
      if (BitSelect(*data_, 11, 6) & 0x2) {
        reg->ddr_load_type.push_back(OPU_MEM_ID_WGT);
      }
      if (BitSelect(*data_, 11, 6) & 0x4) {
        reg->ddr_load_type.push_back(OPU_MEM_ID_BIAS);
      }
      if (BitSelect(*data_, 11, 6) & 0x8) {
        reg->ddr_load_type.push_back(OPU_MEM_ID_RESIDUAL);
      }
      if (BitSelect(*data_, 11, 6) & 0x10) {
        reg->ddr_load_type.push_back(OPU_MEM_ID_INS);
      }
      if (BitSelect(*data_, 11, 6) & 0x20) {
        reg->ddr_load_type.push_back(OPU_MEM_ID_PARAM);
      }
      ...
      os << "ddr_load_type: " << BitSelect(*data_, 11, 6) << "\n";
      break;
    case 12:
    ...
    case 16:
      ...
      reg->shift_num_bias = static_cast<int>(BitSelect(*data_, 19, 10));
      // RTL 1 bit only?
    ...
    */
    ...
    case 32:
      ...
      break;
    case 33:
      reg->ddr_param_addr_ini = static_cast<int>(BitSelect(*data_, 30, 6));
      os << "ddr_param_addr_ini: " << reg->ddr_param_addr_ini << "\n";
      break;
    case 34:
      reg->ddr_param_read_num = static_cast<int>(BitSelect(*data_, 20, 6));
      os << "ddr_param_read_num: " << reg->ddr_param_read_num << "\n";
      break;
    case 35:
      reg->psum_write_addr_s = static_cast<int>(BitSelect(*data_, 20, 6));
      os << "psum_write_addr_s: " << reg->psum_write_addr_s << "\n";
      break;
    case 36:
      reg->add_zero = static_cast<bool>(BitSelect(*data_, 6));
      reg->param_ram_id = static_cast<int>(BitSelect(*data_, 8, 7));
      os << "add_zero: " << reg->add_zero << "\n";
      os << "param_ram_id: " << reg->param_ram_id << "\n";
      break;
    case 37:
      reg->ddr_save_pos = static_cast<int>(BitSelect(*data_, 9, 6));
      os << "ddr_save_pos: " << reg->ddr_save_pos << "\n";
      break;
    case 38:
      reg->nvm_output_fraclen = static_cast<int>(BitSelect(*data_, 17, 6));
      reg->nvm_input_fraclen = static_cast<int>(BitSelect(*data_, 29, 18));
      os << "nvm_input_fraclen: " << reg->nvm_input_fraclen << "\n";
      os << "nvm_output_fraclen: " << reg->nvm_output_fraclen << "\n";
      break;
    case 39:
      reg->nvm_beta_fraclen = static_cast<int>(BitSelect(*data_, 17, 6));
      reg->nvm_gamma_fraclen = static_cast<int>(BitSelect(*data_, 29, 18));
      os << "nvm_beta_fraclen: " << reg->nvm_beta_fraclen << "\n";
      os << "nvm_gamma_fraclen: " << reg->nvm_gamma_fraclen << "\n";
      break;
    default:
