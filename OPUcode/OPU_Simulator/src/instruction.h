...
class OPURegFile {
 public:
  ...
  int se_fc_out_num;
  // new
  int input_addr_s {0};  // DRAM -> input_buffer[w_addr]
  int psum_write_addr_s {0};  // PE -> psum[w_addr]
  int psum_read_addr_s {0};  // psum[r_addr] -> post ops
  int psum_read_stride {0};  // psum[r_addr+i*stride] -> post ops
  bool add_zero;
  int ddr_param_addr_ini;
  int ddr_param_read_num;
  int param_ram_id;
  
  int nvm_input_fraclen;
  int nvm_output_fraclen;
  int nvm_beta_fraclen;
  int nvm_gamma_fraclen;
};
...
class OPUDDRLDInsn : public OPUGenericInsn {
 public:
  ...
  void ReadFrom(OPURegFile* reg) {
    ...
    ddr_ins_addr_ini = reg->ddr_ins_addr_ini;
    ddr_param_addr_ini = reg->ddr_param_addr_ini;
    ddr_param_read_num = reg->ddr_param_read_num;
    param_ram_id = reg->param_ram_id;
    psum_addr_s = reg->psum_write_addr_s;
  }
};
...
class OPUComputeInsn : public OPUGenericInsn {
 public:
  ...
  void ReadFrom(OPURegFile* reg) {
    ...
    input_addr_s = reg->input_addr_s;
    psum_addr_s = reg->psum_write_addr_s;
    add_zero = reg->add_zero;
  }
  ...
  void ResidueSwitchUpdate(int rid) {
    res_ram_id = rid;
  }
};
...
class OPUDDRSTInsn : public OPUGenericInsn {
 public:
  ...
  int out_y_stride;
  int out_x_stride;
  // psum read address offset
  int psum_addr_s;
  int psum_read_stride;
  // fraclen (for simulation only) not in registers
  int nvm_input_fraclen;
  int nvm_output_fraclen;
  int nvm_beta_fraclen;
  int nvm_gamma_fraclen;
  ...
  void ReadFrom(OPURegFile* reg) {
    ...
    psum_addr_s = reg->psum_read_addr_s;
    psum_read_stride = reg->psum_read_stride;

    nvm_input_fraclen = reg->nvm_input_fraclen;
    nvm_output_fraclen = reg->nvm_output_fraclen;
    nvm_beta_fraclen = reg->nvm_beta_fraclen;
    nvm_gamma_fraclen = reg->nvm_gamma_fraclen;
  }
};
